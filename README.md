...
- A Server Side Rendering React Framework for production.
- It offers a lot to make large scale production app easier.
- NextJs is a fullstack framework for ReactJs.
...
npm run dev : It will spin up the dev server which will serve our nextjs our app and auto reload page as we save.
npm run build : Once we are ready to deploy we use this command for deployment.
npm run start : To start the optimize server we use this later on.
...
- By default, NextJs pre-render all pages (~ server-side rendering :SSR)
- In nextjs when a request is made to /some-route it will return a pre-rendered page. whereas in react it will return a html and javascript file and then javascript will be executed and the data will be fetch.
- Nextjs pre-rendered page is Good for Seo as it prepare a page in advance.
- It only affect the initial load.
...
- Nextjs Support dynamic `import()` for Js.
- With it you can import Js modules (React Components dynamically and work with them.)
- They also work with SSR.
- When u need dynamic import for example if u don't want this component to load at the time of page load then use dynamic import.

Example :- Static import : import Header from './header'
